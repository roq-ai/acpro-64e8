...
import { getFinancialRecords } from 'apiSdk/financial-records';
...
function DashboardPage() {
  ...
  const financialRecordStat = useSWR<PaginatedInterface<FinancialRecordInterface>>(
    () => `/financial-records`,
    () => getFinancialRecords({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      ...
      result: financialRecordStat?.data?.totalCount,
      isLoading: financialRecordStat.isLoading,
